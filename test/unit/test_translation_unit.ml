...
let test_parse_and_format ~fg test_name ~input ~expected =
  let test_name = Stdlib.Format.sprintf "parse_and_format: %s" test_name in
  ( test_name
  , `Quick
  , fun () ->
      let actual =
        ...
        |> Result.map_error ~f:(fun e ->
               Stdlib.Format.asprintf "%a" Translation_unit.Error.pp e )
      in
      Alcotest.(check (result string string)) test_name expected actual )

let test_parse_and_format_core_type =
  let make_test = test_parse_and_format ~fg:Core_type in
